...
public class Receivepacket {
    ...
    private final short FLAG_HANDBREAK = 4;
    ...
    */
        ...
        //id (int) at offset 92: mirrors the packet id sent by the app, used
        // by BeamNG's remoteController to tag each telemetry package.
        if (data.length >= 96) {
            id = Integer.reverseBytes(bb.getInt(92));
        }

        //odometer (unsigned int) at offset 96, only present in extended structs.
        if (data.length >= 100) {
            odometer = Integer.reverseBytes(bb.getInt(96));
        }
    ...
    public boolean[] getFlagsArray(){
        java.util.Arrays.fill(flagsarray, false);
    ...
    public boolean[] getDashUsedArr(){
        java.util.Arrays.fill(dasharray, false);
        if((dashLights & FLAG_SHIFTLIGHT) == FLAG_SHIFTLIGHT) { dasharray[0]= true; }
        if((dashLights & FLAG_FULLBEAM) == FLAG_FULLBEAM) { dasharray[1]= true; }
        if((dashLights & FLAG_HANDBREAK) == FLAG_HANDBREAK) { dasharray[2]= true; }
        if((dashLights & FLAG_SIGNAL_L) == FLAG_SIGNAL_L) { dasharray[5]= true; }
        if((dashLights & FLAG_SIGNAL_R) == FLAG_SIGNAL_R) { dasharray[6]= true; }
        if((dashLights & FLAG_SIGNAL_ANY) == FLAG_SIGNAL_ANY) { dasharray[7]= true; }
        if((dashLights & FLAG_OILWARN) == FLAG_OILWARN) { dasharray[8]= true; }
        if((dashLights & FLAG_BATTERY) == FLAG_BATTERY) { dasharray[9]= true; }
        if((dashLights & FLAG_ABS) == FLAG_ABS) { dasharray[10]= true; }
        return dasharray;
    }

    public boolean[] getActiveLightsArr(){
        java.util.Arrays.fill(lightsArray, false);
